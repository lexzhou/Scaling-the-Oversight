...
for model_id in ["meta-llama/Llama-2-7b-chat-hf"]:
    ...
    model = AutoModelForCausalLM.from_pretrained(model_id, quantization_config=bnb_config, device_map={"0": [0, 1, 2, 3, 4, 5], "1": [6, 7, 8, 9, 10, 11]}, use_auth_token=token)
    print(111)
    ...
    model_part1 = nn.Sequential(*list(model.children())[:6])
    model_part2 = nn.Sequential(*list(model.children())[6:])
        
    # Create the pipeline
    print(333)
    model = Pipe(torch.nn.Sequential(model_part1, model_part2), chunks=8)
    ...
    for i in range(1, 2): # Do 2 epochs
        try:
